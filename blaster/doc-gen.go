...
var doc = map[string]string{
	...
	"Config.Data":            "Data sets the the data file to load. Load a local file or stream directly from a GCS bucket with `gs://{bucket}/{filename}.csv`. Data should be in csv format, and if `headers` is not specified the first record will be used as the headers. If a newline character is found, this string is read as the data.",
	...
	"Config.Rate":            "Rate sets the initial rate in requests per second. Simply enter a new rate during execution to adjust this.",
	...
	"Config.Timeout":         "Timeout sets the deadline in the context passed to the worker. The default value is 1000ms. Workers must respect this the context cancellation. We exit with an error if any worker is processing for timeout + 1 second.",
	...
	"Config.Workers":         "Workers sets the number of concurrent workers.",
	"CsvReader":              "",
	"CsvWriteFlusher":        "",
	"DummyCloser":            "",
	"ExampleWorker":          "",
